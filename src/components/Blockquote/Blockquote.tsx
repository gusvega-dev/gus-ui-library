...
const BlockquoteWithImage: React.FC<BlockquoteProps> = ({
  ...
  // Note: This component requires next/image. For standalone use, consider using <img>
  return (
    <figure
      className={`grid grid-cols-[auto,1fr] items-center gap-x-4 gap-y-8 sm:grid-cols-12 sm:grid-rows-[1fr,auto,auto,1fr] sm:gap-x-10 lg:gap-x-16 ${className}`.trim()}
    >
...
const BlockquoteWithoutImage: React.FC<BlockquoteProps> = ({
  author,
  children,
  className = '',
}) => {
  return (
    <div className={`border-l-2 border-neutral-200 pl-8 ${className}`.trim()}>
